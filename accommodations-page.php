...
<div data-pw-id="main-column">
  ...
  <?php if (count($page->inner_page_slideshow)>=1): ?>
    <div  class="innerPageBannerSlideshow cycle-slideshow innerPageAccomodationsSlideshow" data-cycle-fx="fade"  data-cycle-timeout="3000" data-cycle-speed="1000" data-cycle-swipe="true" data-cycle-swipe-fx="scrollHorz" data-cycle-pager=".cycle-pager">
    <?php foreach ($page->inner_page_slideshow as $slide): ?>			
      <img src="<?= $slide->getCrop("slideshow_size")->url; ?>" alt="<?= $slide->description ?>" />
  ...
  <?php endif ?>
    ...
    <?php else :?>
      <?= $page->body ?>
      <?php if ($page->parent->id == "1003"): ?>
        <p><a href="https://irm.cypresscoveresort.net/IRMNet/(S(gaoazpb1aoq54jyptvzfrjjb))/login.aspx?Resort=01" class="checkRatesBtn" target="_blank">Check Rates &amp; Availability</a></p>
      <?php endif ?>
